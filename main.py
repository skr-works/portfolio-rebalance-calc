...
def dividend_yield_to_ratio(value):
    """
    配当利回りを計算用の小数に変換する。
    - yfinance から 2.7 のように%値で入る場合は 0.027 にする
    - 0.027 のように小数で入る場合はそのまま使う
    - 表示用のO列・キャッシュ値は変更しない
    """
...
def main():
    ...
    for row in rows:
        ...
        dy = dividend_yield_to_ratio(div_yield_map.get(tk, 0.0))
